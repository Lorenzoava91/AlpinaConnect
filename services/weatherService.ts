...
export const fetchWeatherForecast = async (lat: number, lng: number, dateStr: string): Promise<WeatherData | null> => {
  try {
    ...
    // Open-Meteo free API usually provides 7 days forecast. 
    // We strictly respect the requirement: show only if distance <= 14 days (Open-Meteo can do up to 16 days in some endpoints, but let's stick to safe 7-10 range for forecast).
    // If date is in the past or too far in future, return null.
